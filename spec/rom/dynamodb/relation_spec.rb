  ...
  describe 'Key: { host<HASH>, logged_at<RANGE> } table' do
    ...
    describe '#equal' do
      describe 'exact' do
        ...
        specify { expect(relation.by_host(host).by_logged_at(time).to_a.size).to eq 1 }
      end

      describe 'none' do
        let(:time) { 0 }

        specify { expect(relation.by_host(host).by_logged_at(time).to_a.size).to eq 0 }
      end
    end
    ...
    describe '#between' do
      ...
      describe 'most' do
        ...
        specify { expect(relation.by_host(host).logged_at_between(after, before).to_a.size).to eq log_range.size }
    ...
    describe '#before' do
      describe 'all' do
        ...
        describe 'with limit' do
          ...
          specify { expect(relation.by_host(host).logged_at_before(time).limit(limit).to_a.size).to eq limit }
        end
      end
      ...
      describe 'some' do
        ...
        specify { expect(relation.by_host(host).logged_at_before(time).to_a.size).to eq log_range.size }
    ...
    describe '#after' do
      describe 'all' do
        ...
        describe 'with limit' do
          ...
          specify { expect(relation.by_host(host).logged_at_after(time).limit(limit).to_a.size).to eq limit }
        end
      end
      ...
      describe 'some' do
        ...
        specify { expect(relation.by_host(host).logged_at_after(time).to_a.size).to eq log_range.size }
